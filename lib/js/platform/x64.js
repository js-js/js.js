...
X64.prototype.genProc = function genProc(instr) {
  var masm = this.masm();

  // Align stack on 16-byte boundary
  var spills = instr.inputs[0].value;
  if (spills % 2 !== 0)
    spills++;
...
X64.prototype.genCallStub = function genCallStub(instr) {
  ...
  if (stub.ic && this.ctx.options.type !== 'ic')
    masm.mov(this.scratch, this.genIC(stub).ptr());
  else
