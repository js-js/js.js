...
X64.prototype.doToBoolean = function doToBoolean(instr) {
...
X64.prototype.doBranch = function doBranch(instr) {
  var masm = this.masm();
...
X64.prototype.doNop = function doNop(instr) {
  if (instr.output.id !== instr.inputs[0].id)
    this.masm().mov(instr.output.id, instr.inputs[0].id);
};

X64.prototype.doLoadArg = function doLoadArg(instr) {
  var masm = this.masm();
  ...
  masm.labelScope(function() {
    this.cmp([ 'rbp', self.ptrSize * 3 ], index << heap.tagShift);

    this.j('le', 'undef');

    this.mov(out, [ 'rbp', self.ptrSize * (4 + index) ]);
    this.j('join');
...
X64.prototype.doFn = function doFn(instr) {
...
X64.prototype.doCheckMap = function doCheckMap(instr) {
  var masm = this.masm();
...
X64.prototype.doPushArg = function doPushArg(instr) {
  var masm = this.masm();
  masm.push(instr.inputs[0].id);
};

X64.prototype.doAlignStack = function doAlignStack(instr) {
  var masm = this.masm();
...
X64.prototype.doCall = function doCall(instr) {
  var Function = heap.entities.Function;
  ...
  // Set new `this`
  var self = masm.heapPtr(this.context, Context.offsets.global);
  masm.push(self);
  masm.mov(self, instr.inputs[1].id);
  ...
  // self was already pop-ed
  size--;
  masm.add('rsp', this.ptrSize * size);
};

X64.prototype.genGoto = function genGoto(succ) {
  if (succ.length === 0)
    return;

  var masm = this.masm();
  if (succ.length === 1)
    return masm.jl(succ[0]);
