...
GetPropertySlot.prototype.miss = function miss(map, prop, result) {
  ...
  probes.prepend(map, prop, result);
...
NormalProbes.prototype.forEach = function forEach(cb, ctx) {
  for (var i = 0, index = 0; i < this.length; i += 3, index++)
    cb.call(ctx, this.list[i + 0], this.list[i + 1], this.list[i + 2], index);
};

NormalProbes.prototype.prepend = function prepend(map, property, value) {
  this.list.unshift(map, property, value);
  this.length = this.list.length;
};
...
FixedProbes.prototype.forEach = function forEach(cb, ctx) {
  for (var i = 0, index = 0; i < this.length; i += 2, index++)
    cb.call(ctx, this.list[i + 0], null, this.list[i + 1], index);
};

FixedProbes.prototype.prepend = function prepend(map, property, value) {
  this.list.unshift(map, value);
  this.length = this.list.length;
};
