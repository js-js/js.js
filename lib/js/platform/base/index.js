...
Base.prototype.replaceFn = function replaceFn(instr) {
  ...
  var block = this.compiler.blocks[instr.inputs[0].id].cfg[0];

  var slots = this.instr('literal', [ state.createJS(block.contextSize) ]);
  var self = this.instr('self', []);
  var code = this.instr('code', instr.removeAllInputs());
  var arg2 = this.instr('pushArg', [ slots ]);
  var arg1 = this.instr('pushArg', [ self ]);
  var arg0 = this.instr('pushArg', [ code ]);
  var call = this.instr('callStub', [
    state.createJS('stub'),
  ...
  return {
    list: [ slots, code, self, arg2, arg1, arg0, call ],
    replace: call
  };
};
