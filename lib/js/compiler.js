...
Compiler.prototype.compileCFG = function compileCFG(cfg, options) {
  if (typeof cfg === 'function') {
    ...
    cfg = ir.parse(cfg, options && options.locals || {});
  }
  ...
  return this.generate(blocks, this.linearscan.spillCount(), options);
};
...
Compiler.prototype.generate = function generate(blocks, spillCount, options) {
  ...
  return this.masmCtx(function() {
    // Do not wrap ICs in Proc
    if (options && options.type === 'ic')
      body();
    else
      this.platform.genProc(spillCount, body);
